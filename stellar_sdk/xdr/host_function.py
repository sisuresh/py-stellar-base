...
class HostFunction:
    ...
    def __hash__(self):
        return hash((self.type, self.invoke_contract, self.create_contract, self.wasm,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.type== other.type and self.invoke_contract== other.invoke_contract and self.create_contract== other.create_contract and self.wasm== other.wasm
    def __str__(self):
        out = []
        out.append(f'type={self.type}')
        out.append(f'invoke_contract={self.invoke_contract}') if self.invoke_contract is not None else None
        out.append(f'create_contract={self.create_contract}') if self.create_contract is not None else None
