...
class OperationBody:
    ...
    def pack(self, packer: Packer) -> None:
        ...
        if self.type == OperationType.BEGIN_SPONSORING_FUTURE_RESERVES:
            if self.begin_sponsoring_future_reserves_op is None:
                raise ValueError("begin_sponsoring_future_reserves_op should not be None.")
    ...
    def unpack(cls, unpacker: Unpacker) -> OperationBody:
        ...
        if type == OperationType.PATH_PAYMENT_STRICT_RECEIVE:
            ...
            return cls(type=type, path_payment_strict_receive_op=path_payment_strict_receive_op)
        ...
        if type == OperationType.CREATE_PASSIVE_SELL_OFFER:
            ...
            return cls(type=type, create_passive_sell_offer_op=create_passive_sell_offer_op)
        ...
        if type == OperationType.PATH_PAYMENT_STRICT_SEND:
            ...
            return cls(type=type, path_payment_strict_send_op=path_payment_strict_send_op)
        if type == OperationType.CREATE_CLAIMABLE_BALANCE:
            create_claimable_balance_op = CreateClaimableBalanceOp.unpack(unpacker)
            return cls(type=type, create_claimable_balance_op=create_claimable_balance_op)
        ...
        if type == OperationType.BEGIN_SPONSORING_FUTURE_RESERVES:
            begin_sponsoring_future_reserves_op = BeginSponsoringFutureReservesOp.unpack(unpacker)
            return cls(type=type, begin_sponsoring_future_reserves_op=begin_sponsoring_future_reserves_op)
        ...
        if type == OperationType.CLAWBACK_CLAIMABLE_BALANCE:
            ...
            return cls(type=type, clawback_claimable_balance_op=clawback_claimable_balance_op)
        ...
        if type == OperationType.BUMP_FOOTPRINT_EXPIRATION:
            ...
            return cls(type=type, bump_footprint_expiration_op=bump_footprint_expiration_op)
    ...
    def __str__(self):
        ...
        out.append(f'path_payment_strict_receive_op={self.path_payment_strict_receive_op}') if self.path_payment_strict_receive_op is not None else None
        out.append(f'manage_sell_offer_op={self.manage_sell_offer_op}') if self.manage_sell_offer_op is not None else None
        out.append(f'create_passive_sell_offer_op={self.create_passive_sell_offer_op}') if self.create_passive_sell_offer_op is not None else None
        ...
        out.append(f'path_payment_strict_send_op={self.path_payment_strict_send_op}') if self.path_payment_strict_send_op is not None else None
        out.append(f'create_claimable_balance_op={self.create_claimable_balance_op}') if self.create_claimable_balance_op is not None else None
        out.append(f'claim_claimable_balance_op={self.claim_claimable_balance_op}') if self.claim_claimable_balance_op is not None else None
        out.append(f'begin_sponsoring_future_reserves_op={self.begin_sponsoring_future_reserves_op}') if self.begin_sponsoring_future_reserves_op is not None else None
        out.append(f'revoke_sponsorship_op={self.revoke_sponsorship_op}') if self.revoke_sponsorship_op is not None else None
        out.append(f'clawback_op={self.clawback_op}') if self.clawback_op is not None else None
        out.append(f'clawback_claimable_balance_op={self.clawback_claimable_balance_op}') if self.clawback_claimable_balance_op is not None else None
        out.append(f'set_trust_line_flags_op={self.set_trust_line_flags_op}') if self.set_trust_line_flags_op is not None else None
        out.append(f'liquidity_pool_deposit_op={self.liquidity_pool_deposit_op}') if self.liquidity_pool_deposit_op is not None else None
        out.append(f'liquidity_pool_withdraw_op={self.liquidity_pool_withdraw_op}') if self.liquidity_pool_withdraw_op is not None else None
        out.append(f'invoke_host_function_op={self.invoke_host_function_op}') if self.invoke_host_function_op is not None else None
        out.append(f'bump_footprint_expiration_op={self.bump_footprint_expiration_op}') if self.bump_footprint_expiration_op is not None else None
        out.append(f'restore_footprint_op={self.restore_footprint_op}') if self.restore_footprint_op is not None else None
