...
class SorobanAuthorizedFunction:
    ...
    def pack(self, packer: Packer) -> None:
        ...
        if self.type == SorobanAuthorizedFunctionType.SOROBAN_AUTHORIZED_FUNCTION_TYPE_CONTRACT_FN:
            ...
            return
        if self.type == SorobanAuthorizedFunctionType.SOROBAN_AUTHORIZED_FUNCTION_TYPE_CREATE_CONTRACT_HOST_FN:
    ...
    def unpack(cls, unpacker: Unpacker) -> SorobanAuthorizedFunction:
        ...
        if type == SorobanAuthorizedFunctionType.SOROBAN_AUTHORIZED_FUNCTION_TYPE_CREATE_CONTRACT_HOST_FN:
    ...
    def __hash__(self):
        return hash((self.type, self.contract_fn, self.create_contract_host_fn,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.type== other.type and self.contract_fn== other.contract_fn and self.create_contract_host_fn== other.create_contract_host_fn
    def __str__(self):
        out = []
        out.append(f'type={self.type}')
        out.append(f'contract_fn={self.contract_fn}') if self.contract_fn is not None else None
        out.append(f'create_contract_host_fn={self.create_contract_host_fn}') if self.create_contract_host_fn is not None else None
