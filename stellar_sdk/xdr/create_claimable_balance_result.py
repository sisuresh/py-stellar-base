...
class CreateClaimableBalanceResult:
    ...
    def pack(self, packer: Packer) -> None:
        ...
        if self.code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_SUCCESS:
            ...
            return
        if self.code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_MALFORMED:
            return
        if self.code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_LOW_RESERVE:
            return
        if self.code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_NO_TRUST:
            return
        if self.code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_NOT_AUTHORIZED:
            return
        if self.code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_UNDERFUNDED:
            return
    @classmethod
    def unpack(cls, unpacker: Unpacker) -> CreateClaimableBalanceResult:
        ...
        if code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_LOW_RESERVE:
            return cls(code=code)
        if code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_NO_TRUST:
            return cls(code=code)
        if code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_NOT_AUTHORIZED:
            return cls(code=code)
        if code == CreateClaimableBalanceResultCode.CREATE_CLAIMABLE_BALANCE_UNDERFUNDED:
            return cls(code=code)
        return cls(code=code)
    ...
    def __hash__(self):
        return hash((self.code, self.balance_id,))
    ...
    def __str__(self):
        ...
        out.append(f'balance_id={self.balance_id}') if self.balance_id is not None else None
