...
class ContractCostParams:
    ...
    def __init__(self, contract_cost_params: List[ContractCostParamEntry]) -> None:
        ...
        if contract_cost_params and len(contract_cost_params) > _expect_max_length:
            raise ValueError(f"The maximum length of `contract_cost_params` should be {_expect_max_length}, but got {len(contract_cost_params)}.")
    ...
    def __str__(self):
        return f"<ContractCostParams [contract_cost_params={self.contract_cost_params}]>"
