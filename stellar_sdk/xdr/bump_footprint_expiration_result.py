...
class BumpFootprintExpirationResult:
    ...
    def pack(self, packer: Packer) -> None:
        ...
        if self.code == BumpFootprintExpirationResultCode.BUMP_FOOTPRINT_EXPIRATION_SUCCESS:
            return
        if self.code == BumpFootprintExpirationResultCode.BUMP_FOOTPRINT_EXPIRATION_MALFORMED:
            return
        if self.code == BumpFootprintExpirationResultCode.BUMP_FOOTPRINT_EXPIRATION_RESOURCE_LIMIT_EXCEEDED:
            return
        if self.code == BumpFootprintExpirationResultCode.BUMP_FOOTPRINT_EXPIRATION_INSUFFICIENT_REFUNDABLE_FEE:
            return
    @classmethod
    def unpack(cls, unpacker: Unpacker) -> BumpFootprintExpirationResult:
        ...
        if code == BumpFootprintExpirationResultCode.BUMP_FOOTPRINT_EXPIRATION_MALFORMED:
            return cls(code=code)
        if code == BumpFootprintExpirationResultCode.BUMP_FOOTPRINT_EXPIRATION_RESOURCE_LIMIT_EXCEEDED:
            return cls(code=code)
        if code == BumpFootprintExpirationResultCode.BUMP_FOOTPRINT_EXPIRATION_INSUFFICIENT_REFUNDABLE_FEE:
            return cls(code=code)
        return cls(code=code)
