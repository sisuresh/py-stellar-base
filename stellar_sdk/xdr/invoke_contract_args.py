...
class InvokeContractArgs:
    ...
    def __init__(
        ...
        if args and len(args) > _expect_max_length:
            raise ValueError(f"The maximum length of `args` should be {_expect_max_length}, but got {len(args)}.")
    ...
    def __hash__(self):
        return hash((self.contract_address, self.function_name, self.args,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.contract_address== other.contract_address and self.function_name== other.function_name and self.args== other.args
    def __str__(self):
        out = [
