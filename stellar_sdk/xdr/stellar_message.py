...
class StellarMessage:
    ...
    def __init__(
        ...
        if peers and len(peers) > _expect_max_length:
            raise ValueError(f"The maximum length of `peers` should be {_expect_max_length}, but got {len(peers)}.")
        self.type = type
        self.error = error
        self.hello = hello
        self.auth = auth
    ...
    def unpack(cls, unpacker: Unpacker) -> StellarMessage:
        ...
        if type == MessageType.SURVEY_REQUEST:
            ...
            return cls(type=type, signed_survey_request_message=signed_survey_request_message)
        if type == MessageType.SURVEY_RESPONSE:
            signed_survey_response_message = SignedSurveyResponseMessage.unpack(unpacker)
            return cls(type=type, signed_survey_response_message=signed_survey_response_message)
    ...
    def __str__(self):
        ...
        out.append(f'signed_survey_request_message={self.signed_survey_request_message}') if self.signed_survey_request_message is not None else None
        out.append(f'signed_survey_response_message={self.signed_survey_response_message}') if self.signed_survey_response_message is not None else None
        ...
        out.append(f'send_more_message={self.send_more_message}') if self.send_more_message is not None else None
        out.append(f'send_more_extended_message={self.send_more_extended_message}') if self.send_more_extended_message is not None else None
        out.append(f'flood_advert={self.flood_advert}') if self.flood_advert is not None else None
        out.append(f'flood_demand={self.flood_demand}') if self.flood_demand is not None else None
