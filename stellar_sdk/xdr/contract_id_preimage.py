...
class ContractIDPreimage:
    ...
    def __hash__(self):
        return hash((self.type, self.from_address, self.from_asset,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.type== other.type and self.from_address== other.from_address and self.from_asset== other.from_asset
    def __str__(self):
        out = []
        out.append(f'type={self.type}')
        out.append(f'from_address={self.from_address}') if self.from_address is not None else None
        out.append(f'from_asset={self.from_asset}') if self.from_asset is not None else None
