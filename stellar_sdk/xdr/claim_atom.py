...
class ClaimAtom:
    ...
    def __hash__(self):
        return hash((self.type, self.v0, self.order_book, self.liquidity_pool,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.type== other.type and self.v0== other.v0 and self.order_book== other.order_book and self.liquidity_pool== other.liquidity_pool
    def __str__(self):
        out = []
        out.append(f'type={self.type}')
        out.append(f'v0={self.v0}') if self.v0 is not None else None
        out.append(f'order_book={self.order_book}') if self.order_book is not None else None
