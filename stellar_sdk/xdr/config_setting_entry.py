...
class ConfigSettingEntry:
    ...
    def __init__(
        ...
        if bucket_list_size_window and len(bucket_list_size_window) > _expect_max_length:
            raise ValueError(f"The maximum length of `bucket_list_size_window` should be {_expect_max_length}, but got {len(bucket_list_size_window)}.")
    ...
    def pack(self, packer: Packer) -> None:
        ...
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_MAX_SIZE_BYTES:
        ...
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_COMPUTE_V0:
            ...
            return
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_LEDGER_COST_V0:
            ...
            return
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_HISTORICAL_DATA_V0:
        ...
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_BANDWIDTH_V0:
            ...
            return
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_COST_PARAMS_CPU_INSTRUCTIONS:
        ...
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_COST_PARAMS_MEMORY_BYTES:
            ...
            return
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_DATA_KEY_SIZE_BYTES:
            ...
            return
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_DATA_ENTRY_SIZE_BYTES:
        ...
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_STATE_EXPIRATION:
            ...
            return
        if self.config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_EXECUTION_LANES:
    ...
    def unpack(cls, unpacker: Unpacker) -> ConfigSettingEntry:
        ...
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_MAX_SIZE_BYTES:
            ...
            return cls(config_setting_id=config_setting_id, contract_max_size_bytes=contract_max_size_bytes)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_COMPUTE_V0:
            ...
            return cls(config_setting_id=config_setting_id, contract_compute=contract_compute)
        ...
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_HISTORICAL_DATA_V0:
            contract_historical_data = ConfigSettingContractHistoricalDataV0.unpack(unpacker)
            return cls(config_setting_id=config_setting_id, contract_historical_data=contract_historical_data)
        ...
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_BANDWIDTH_V0:
            ...
            return cls(config_setting_id=config_setting_id, contract_bandwidth=contract_bandwidth)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_COST_PARAMS_CPU_INSTRUCTIONS:
            contract_cost_params_cpu_insns = ContractCostParams.unpack(unpacker)
            return cls(config_setting_id=config_setting_id, contract_cost_params_cpu_insns=contract_cost_params_cpu_insns)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_COST_PARAMS_MEMORY_BYTES:
            contract_cost_params_mem_bytes = ContractCostParams.unpack(unpacker)
            return cls(config_setting_id=config_setting_id, contract_cost_params_mem_bytes=contract_cost_params_mem_bytes)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_DATA_KEY_SIZE_BYTES:
            contract_data_key_size_bytes = Uint32.unpack(unpacker)
            return cls(config_setting_id=config_setting_id, contract_data_key_size_bytes=contract_data_key_size_bytes)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_DATA_ENTRY_SIZE_BYTES:
            contract_data_entry_size_bytes = Uint32.unpack(unpacker)
            return cls(config_setting_id=config_setting_id, contract_data_entry_size_bytes=contract_data_entry_size_bytes)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_STATE_EXPIRATION:
            ...
            return cls(config_setting_id=config_setting_id, state_expiration_settings=state_expiration_settings)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_CONTRACT_EXECUTION_LANES:
            contract_execution_lanes = ConfigSettingContractExecutionLanesV0.unpack(unpacker)
            return cls(config_setting_id=config_setting_id, contract_execution_lanes=contract_execution_lanes)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_BUCKETLIST_SIZE_WINDOW:
            ...
            return cls(config_setting_id=config_setting_id, bucket_list_size_window=bucket_list_size_window)
        if config_setting_id == ConfigSettingID.CONFIG_SETTING_EVICTION_ITERATOR:
            eviction_iterator = EvictionIterator.unpack(unpacker)
            return cls(config_setting_id=config_setting_id, eviction_iterator=eviction_iterator)
    ...
    def __hash__(self):
        return hash((self.config_setting_id, self.contract_max_size_bytes, self.contract_compute, self.contract_ledger_cost, self.contract_historical_data, self.contract_events, self.contract_bandwidth, self.contract_cost_params_cpu_insns, self.contract_cost_params_mem_bytes, self.contract_data_key_size_bytes, self.contract_data_entry_size_bytes, self.state_expiration_settings, self.contract_execution_lanes, self.bucket_list_size_window, self.eviction_iterator,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.config_setting_id== other.config_setting_id and self.contract_max_size_bytes== other.contract_max_size_bytes and self.contract_compute== other.contract_compute and self.contract_ledger_cost== other.contract_ledger_cost and self.contract_historical_data== other.contract_historical_data and self.contract_events== other.contract_events and self.contract_bandwidth== other.contract_bandwidth and self.contract_cost_params_cpu_insns== other.contract_cost_params_cpu_insns and self.contract_cost_params_mem_bytes== other.contract_cost_params_mem_bytes and self.contract_data_key_size_bytes== other.contract_data_key_size_bytes and self.contract_data_entry_size_bytes== other.contract_data_entry_size_bytes and self.state_expiration_settings== other.state_expiration_settings and self.contract_execution_lanes== other.contract_execution_lanes and self.bucket_list_size_window== other.bucket_list_size_window and self.eviction_iterator== other.eviction_iterator
    def __str__(self):
        out = []
        out.append(f'config_setting_id={self.config_setting_id}')
        out.append(f'contract_max_size_bytes={self.contract_max_size_bytes}') if self.contract_max_size_bytes is not None else None
        out.append(f'contract_compute={self.contract_compute}') if self.contract_compute is not None else None
        out.append(f'contract_ledger_cost={self.contract_ledger_cost}') if self.contract_ledger_cost is not None else None
        out.append(f'contract_historical_data={self.contract_historical_data}') if self.contract_historical_data is not None else None
        out.append(f'contract_events={self.contract_events}') if self.contract_events is not None else None
        out.append(f'contract_bandwidth={self.contract_bandwidth}') if self.contract_bandwidth is not None else None
        out.append(f'contract_cost_params_cpu_insns={self.contract_cost_params_cpu_insns}') if self.contract_cost_params_cpu_insns is not None else None
        out.append(f'contract_cost_params_mem_bytes={self.contract_cost_params_mem_bytes}') if self.contract_cost_params_mem_bytes is not None else None
        out.append(f'contract_data_key_size_bytes={self.contract_data_key_size_bytes}') if self.contract_data_key_size_bytes is not None else None
        out.append(f'contract_data_entry_size_bytes={self.contract_data_entry_size_bytes}') if self.contract_data_entry_size_bytes is not None else None
        out.append(f'state_expiration_settings={self.state_expiration_settings}') if self.state_expiration_settings is not None else None
        out.append(f'contract_execution_lanes={self.contract_execution_lanes}') if self.contract_execution_lanes is not None else None
        out.append(f'bucket_list_size_window={self.bucket_list_size_window}') if self.bucket_list_size_window is not None else None
        out.append(f'eviction_iterator={self.eviction_iterator}') if self.eviction_iterator is not None else None
